...
class Auth extends CI_Controller
{
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // untuk mengirim email private function
    ...
    {
        $config = [
            //simpel main transfer protocol
            'protocol' => 'smtp',
            'smtp_host' => 'ssl://smtp.googlemail.com',
            'smtp_user' => 'muangsana2521@gmail.com',
            'smtp_pass' => 'Aselole123',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n"
        ...
        $this->email->from('muangsana2521@gmail.com', 'Muang Sana');
    ...
    // memverifikasi url dari email
    public function verify()
    {
        $email = $this->input->get('email');
        $token = $this->input->get('token');

        // mengambil tabel user sesuai dengan email pada url
        $user = $this->db->get_where('user', ['email' => $email])->row_array();

        //mengecek email jika ada
        if ($user) {
            $user_token = $this->db->get_where('user_token', ['token' => $token])->row_array();
            //mengecek token jika ada
            if ($user_token) {
                //mengecek waktu 
    ...
    public function resetPassword()
    {
        ...
        $user = $this->db->get_where('user', ['email' => $email])->row_array();
        // mengecek email sesuai dengan input email jika ada
        if ($user) {
            $user_token = $this->db->get_where('user_token', ['token' => $token])->row_array();
            // mengecek token sesuai dengan input email jika ada
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 1)) {
                    // masuk membawa session
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
            Reset Password failed ' . $email . ' Wrong Email
             </div>');
            redirect('auth');
        }
    }
